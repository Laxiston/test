...
function webpackProgress(compiler, headingMessage) {
    var bar = new ProgressBar(' ' + headingMessage + ' [:bar] :percent :etas : :message', {
        complete: '=',
        incomplete: ' ',
        width: 35,
        total: 100
...
gulp.task('frontend-watch', function (done) {
    ...
    compiler.plugin('done', function (stats) {
        ...
        if (initialCompile) {
            console.log(colors.magenta('Webpack Dev Server listening at localhost:' + appConfig.ports.devServer));
...
gulp.task('watch', ['backend-watch'], function () {
    ...
    nodemon({
        ...
        ignore: ['*'],
        watch: !appConfig.production ? ['build/server/*'] : false,
        ext: 'js'
    }).on('restart', function () {
        if (firstStart) {
            console.log(colors.magenta('Starting express server!'));
            firstStart = false;
        } else {
            console.log(colors.magenta('Restarting express server!'));
        }
    });
});
